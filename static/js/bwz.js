...
if (w <= 769) {
  ...
  if (djangoView == 'category_view'){ 
    ...
    bc.style.marginLeft = leftOffset;
    if (w > 769 && w < 1150){
      postIntroText[0].style.marginTop = ptHeight+45+'px';
    } else {
      postIntroText[0].style.marginTop = ptHeight+65+'px';
    }  
    if (w > 1700){
      postDate[0].style.marginTop = ptHeight+320+'px';
    } else {
      if (w > 769 && w < 1399){
        postDate[0].style.marginTop = ptHeight+250+'px';
      }else if (w > 1399){
